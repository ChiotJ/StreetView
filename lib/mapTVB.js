    ...
    panoInit: function () {
        ...
        this.PANO = new Panorama('panoCon', svid, {
            ...
            onPanoChange: function () {
                ...
                if (lookedScene) {
                    window.sessionStorage.setItem("lookedScene"+ GHSMLib.cardId, lookedScene + 1);
                } else {
                    window.sessionStorage.setItem("lookedScene"+ GHSMLib.cardId, 1);
                }

                
                var b = "http://test.visualbusiness.cn/gehua_pano?pano=" + t.PANO.pano;
                dataLoader && dataLoader.abort(), dataLoader = $.ajax({
                    type: "get",
                    async: false,
                    url: t.PANO.getProxyUrl(b),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    cache: false,
                    success: function (data) {
                        dataLoader = null;
                        t.PANO.planeInfo = data;
                        if (data && data.building) {
                            ...
                            $('#album').css("left", "0");
                            t.photo ?
                                t.photo.changeRegion(data.building.info.current_floor, t.PANO.pano, data) :
                                t.photo = new Photo(t, {
                                    regionId: data.building.info.current_floor,
                                    cellWidth: 186,
                                    cellHeight: 115
                                }, data);
                            t.regeionList ?
                                t.regeionList.setFloorData(data.building.floors) :
                                t.regeionList = new RegionList(t.photo, data.building.floors);
                            var current_floor = data.building.info.current_floor;
                            t.regeionList.selectFloor(current_floor ? current_floor : 0);
                            if (t.first) {
                                t.changeArea(2);
...
//相册
function Photo(a, b, data) {
    this.photoData = null, this.regionId = null, this.container = document.getElementById("albumViewBody"), this.mapT = a, this.pano = a.PANO, this.swipe_photo = null, this.select = null, this.dataLoader = null, this.photoScroll = null, this.cellWidth = b.cellWidth || 59, this.cellPadding = b.cellPadding || 20, this.cellHeight = b.cellHeight || 59, this.photoData = null, this.scrollLeft = 0, this.left = 0;
    this.changeRegion(b.regionId, this.pano.pano, data)
}
var photoP = Photo.prototype;
photoP.changeRegion = function (a, b, data) {
    if (this.regionId != a) {
        this.regionId = a;
        this.curSvid = b;
        this.photoData = data;
...
photoP._createCell = function (a) {
    ...
    $(b).attr("heading", a.heading);
    var c = this, d = "http://capture.map.qq.com/screenshot?model=web&from=qqmap&zoom=0&fov=60&width=" + c.cellWidth + "&height=" + c.cellHeight + "&pano=" + a.svid + "&pitch=" + a.pitch + "&heading=" + a.heading;
