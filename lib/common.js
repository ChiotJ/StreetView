var TXConneturl = "http://" + window.location.host + "/TVMap/transfer?";
...
//            that.PANO = new Panorama('panoCon', svid);
        ...
        /*地图初始化*/
        mapInit: function (t, cardId) {
            var that = this;
            var lat = 39.9489391276258;
            var lng = 116.42458676023809;
            if (cardId) {
                var u = this.getUserPosition(cardId);
                lat = u.map.lat;
                lng = u.map.lng;
            }
            ...
            qq.maps.event.addListener(that.MAP, 'center_changed', function () {
                ...
                if (typeof t.centerChange === "function") {
                    t.centerChange();
                }
            });
...
//                var list = $('#houseList li');
        ...
        //获取用户位置信息
        getUserPosition: function (cardId) {
            var CA = cardId;
            var p = {}, map = {};
            ...
            $.ajax({
                url: "http://172.16.188.26/chaowai/box/getBoxByCardId?cardId=" + CA,
                type: "GET",
                dataType: 'json',
                async: false,
                success: function (data) {
                    if (data.success) {
                        ...
                        p.address = data.result.address;
                        map.lat = data.result.latitude;
                        map.lng = data.result.longitude;
                    }
                }
            });
            p.map = map;
        ...
        user: {
            CardID: typeof CyberCloud != "undefined" ? CyberCloud.GetParam("CardID").ParamValue : "card1"
        }
    }
})();
